    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          const system = `You generate high-quality prompt library suggestions. Reply with STRICT valid JSON only: an array of 6 objects with "title" (2-4 words, Title Case) and "body" (a fully-formed 2-3 sentence prompt a user could paste into a ${kind} generator). No prose, no code fences, no explanations. Prompts must be for ${domain} and must NOT overlap with other generator domains.`;
